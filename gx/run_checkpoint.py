# -*- coding: utf-8 -*-
"""
Great Expectations / Data Quality (compétence C8).
Porte 5 contrôles critiques Silver/Gold vers une suite exécutable.
Usage : python gx/run_checkpoint.py
"""
...
def run_suite() -> dict:
    ...
    results.append(expect("expect_gold_chomage_n1_complete", ok,
                           f"completude={gold['taux_chomage_n1'].notna().mean():.1%}"))

    # Bonus : pauvreté N-1 non totalement vide si historique branché
    ...
    report = {
        ...
        "framework": "great_expectations-compatible custom runner",
    ...
    # Rapport HTML minimal (preuve visuelle C8)
    rows_html = "".join(
...
<body>
<h1>Checkpoint electio_silver_gold</h1>
<p>Success global : <strong class="{'ok' if success else 'ko'}">{success}</strong></p>
<table><thead><tr><th>Expectation</th><th>Status</th><th>Detail</th></tr></thead>
...
<p>Généré : {report['generated_at']}</p>
    ...
    # Tentative d'enregistrement via Great Expectations si installé
    try:
        import great_expectations as gx  # noqa: F401
        report["great_expectations_installed"] = True
    except ImportError:
        report["great_expectations_installed"] = False

    print(json.dumps({k: report[k] for k in ("success", "great_expectations_installed")}, indent=2))
    for r in results:
